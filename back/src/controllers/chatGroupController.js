...
export const chat_group_get = asyncHandler(async (req, res) => {
  ...
  const group = await Group.findById(req.params.groupid, "-__v")
    .populate("creator")
    .exec();
  if (group === null) return res.sendStatus(404);

  // current logged in user reference with group
  const currentUserInGroup = await GroupMember.findOne({
    group,
    user: req.user,
  }).exec();
  ...
  if (isMember) {
    ...
    messages = await Message.find({ groupReceive: group }, "-__v")
...
export const chat_group_post = [
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!errors.length) {
      ...
      // find all messages are being sent to this group
      let messages = await Message.find({ groupReceive: group }, "-__v")
        .populate("sender", "_id avatarLink")
        .sort({ createdAt: 1 })
        .exec();
...
export const chat_group_delete = asyncHandler(async (req, res) => {
  ...
  // check if group we want really exists
  const group = await Group.findById(req.params.groupid, "_id")
...
export const chat_group_put = [
  ...
  asyncHandler(async (req, res) => {
    ...
    // check if group we want really exists
    const oldGroup = await Group.findById(
      req.params.groupid,
      "_id name createdAt",
    )
      .populate("creator", "_id")
    ...
    const countGroupName = await Group.countDocuments({ name }).exec();
    if (countGroupName > 0 && oldGroup.name !== name)
      errors.push({ msg: `Group name exists.` });
...
export const chat_group_member_delete = asyncHandler(async (req, res) => {
  ...
  if (
    (!loggedInUserRefInGroup.isCreator &&
      loggedInUserRefInGroup?.user?.id !== userToDeleteRefInGroup?.user?.id) ||
